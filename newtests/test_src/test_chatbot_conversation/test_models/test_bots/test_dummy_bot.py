...
def test_should_retry_on_exception():
    ...
    assert not bot._should_retry_on_exception(Exception()) # type: ignore
    assert not bot._should_retry_on_exception(ValueError())  # type: ignore
    assert not bot._should_retry_on_exception(RuntimeError())  # type: ignore


def test_generate_response_uses_predefined_responses():
...
def test_model_constants():
    ...
    assert bot._default_temperature == 1.0 # type: ignore
    assert bot._get_default_max_tokens() == 50 # type: ignore
